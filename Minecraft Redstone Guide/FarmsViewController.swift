...
class FarmsViewController: UIViewController, GADRewardedAdDelegate {
    ...
    override func viewDidLoad() {
        ...
        for index in 0...(locked.count - 1) {
            if locked[index] && global.farmsVideoProgress![index] as! Int >= global.numRewardedVideosRequired{
                locked[index] = false
            }
        }
    ...
    func rewardedAd(_ rewardedAd: GADRewardedAd, userDidEarn reward: GADAdReward) {
        ...
        let progress:Int = global.farmsVideoProgress![row] as! Int + 1
        global.farmsVideoProgress?[row] = progress
        UserDefaults.standard.set(global.farmsVideoProgress, forKey: "farmsVideoProgress")
        if global.farmsVideoProgress?[row] as! Int == global.numRewardedVideosRequired {
            locked[row] = false
...
extension FarmsViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if (!global.fullVersion && locked[indexPath.row]) {
            ...
            let remainingVids = global.numRewardedVideosRequired - (global.farmsVideoProgress![indexPath.row] as! Int)
