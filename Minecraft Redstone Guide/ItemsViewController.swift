...
class ItemsViewController: UIViewController, GADRewardedAdDelegate {
    ...
    override func viewDidLoad() {
        ...
        for index in 0...(locked.count - 1) {
            if locked[index] && global.itemsVideoProgress![index] as! Int >= global.numRewardedVideosRequired{
                locked[index] = false
            }
        }
    ...
    func rewardedAd(_ rewardedAd: GADRewardedAd, userDidEarn reward: GADAdReward) {
        ...
        let progress:Int = global.itemsVideoProgress![row] as! Int + 1
        global.itemsVideoProgress?[row] = progress
        UserDefaults.standard.set(global.itemsVideoProgress, forKey: "itemsVideoProgress")
        if global.itemsVideoProgress?[row] as! Int == global.numRewardedVideosRequired {
            locked[row] = false
...
extension ItemsViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if (!global.fullVersion && locked[indexPath.row]) {
            ...
            let remainingVids = global.numRewardedVideosRequired - (global.itemsVideoProgress![indexPath.row] as! Int)
